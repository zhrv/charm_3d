...
#define CHARM_VTK_LOCIDX P4EST_VTK_LOCIDX

/** Opaque context type for writing VTK output with multiple function calls.
 */
typedef struct charm_vtk_context charm_vtk_context_t;
 ...
 * The quadrants are scaled to length .95; see \ref charm_vtk_write_header.
 ...
 */
void                charm_vtk_write_file (p4est_t * p4est,
 ...
 *                  \ref charm_vtk_context_set_geom, we require
 ...
 */
void                charm_vtk_context_set_geom (charm_vtk_context_t * cont,
                                                p4est_geometry_t * geom);

/** Modify the context parameter for scaling the quadrants.
 * After \ref charm_vtk_context_new, it is at the default 0.95.
 * \param [in,out] cont         The context is modified.
 *                              It must not yet have been used to start writing
 *                              in \ref charm_vtk_write_header.
 ...
 * After \ref charm_vtk_context_new, it is at the default false.
 * \param [in,out] cont         The context is modified.
 *                              It must not yet have been used to start writing
 *                              in \ref charm_vtk_write_header.
 * \param [in] continuous       Boolean parameter.
 */
void                charm_vtk_context_set_continuous (charm_vtk_context_t *
                                                      cont, int continuous);

/** Cleanly destroy a \ref charm_vtk_context_t structure.
 *
 ...
 */
void                charm_vtk_context_destroy (charm_vtk_context_t * context);
 ...
 *
 *     vtk_context = charm_vtk_context_new (p4est, "output");
 *     charm_vtk_context_set_* (vtk_context, parameter);
 *     vtk_context = charm_vtk_write_header (vtk_context, ...);
 *     if (vtk_context == NULL) { error; }
 *     vtk_context = charm_vtk_write_cell_data (vtk_context, ...);
 *     if (vtk_context == NULL) { error; }
 *     vtk_context = charm_vtk_write_point_data (vtk_context, ...);
 *     if (vtk_context == NULL) { error; }
 *     retval = charm_vtk_write_footer (vtk_context);
 *     if (retval) { error; }
 *
 ...
 *
 * \return          On success, an opaque context (charm_vtk_context_t) pointer
 *                  that must be passed to subsequent p4est_vtk calls.  It is
 *                  required to call \ref charm_vtk_write_footer eventually with
 *                  this value.  Returns NULL on error.
 */
charm_vtk_context_t *charm_vtk_write_header (charm_vtk_context_t * cont);
 ...
 */
charm_vtk_context_t *charm_vtk_write_cell_dataf (charm_vtk_context_t * cont,
                                                 int write_tree,
                                                 int write_level,
                                                 int write_rank,
                                                 int wrap_rank,
 ...
 *
 * \note The current
 * charm_vtk_context_t structure, cont, must be the last argument of any call
 ...
 *                  On failure, returns NULL and deallocates the context.
 */
charm_vtk_context_t *charm_vtk_write_point_dataf (charm_vtk_context_t * cont,
 ...
 */
int                 charm_vtk_write_footer (charm_vtk_context_t * cont);
