...
static const int    charm_vtk_write_tree = 1;
static const int    charm_vtk_write_level = 1;
static const int    charm_vtk_write_rank = 1;
static const int    charm_vtk_wrap_rank = 0;
 ...
 * When in doubt, please use \ref charm_vtk_write_cell_data instead.
 *
 * \param [in,out] cont    A VTK context created by \ref charm_vtk_context_new.
 ...
 */
static
  charm_vtk_context_t *charm_vtk_write_cell_scalar (charm_vtk_context_t *
                                                    cont,
 ...
 */
static
  charm_vtk_context_t *charm_vtk_write_cell_vector (charm_vtk_context_t *
                                                    cont,
 ...
 * Writing a VTK file is split into a few routines.
 * This allows there to be an arbitrary number of fields.
 * When in doubt, please use \ref charm_vtk_write_point_data instead.
 *
 * \param [in,out] cont    A VTK context created by \ref charm_vtk_context_new.
 * \param [in] scalar_name The name of the scalar field.
 ...
 */
static
  charm_vtk_context_t *charm_vtk_write_point_scalar (charm_vtk_context_t *
                                                     cont,
                                                     const char *scalar_name,
                                                     sc_array_t * values);
 ...
 */
static
  charm_vtk_context_t *charm_vtk_write_point_vector (charm_vtk_context_t *
                                                     cont,
                                                     const char *vector_name,
                                                     sc_array_t * values);

//#ifdef P4_TO_P8
//#define charm_vtk_context               p8est_vtk_context
//#endif
...
#define CHARM_VTK_ASCII 1
#define CHARM_VTK_FORMAT_STRING "ascii"
#else
#define CHARM_VTK_FORMAT_STRING "binary"
...
#endif /* CHARM_VTK_COMPRESSION */
}

#endif /* CHARM_VTK_BINARY */
 ...
 * It is used to relay necessary vtk information to the \b charm_vtk_write_*
 * functions. This structure is initialized by \ref charm_vtk_write_header and
 * destroyed by \b charm_vtk_write_footer; it can also be destroyed manually
 * using the \b charm_vtk_context_destroy function if necessary.
 *
 ...
 * this is determined in \ref charm_vtk_write_header using the \a scale parameter
 ...
 * pointers; opened by \ref charm_vtk_write_header and closed by \b
 * charm_vtk_write_footer.
 *
 */
struct charm_vtk_context
{
  ...
  int                 writing;     /**< True after charm_vtk_write_header. */
...
charm_vtk_context_t *
charm_vtk_context_new (p4est_t * p4est, const char *filename)
{
  charm_vtk_context_t *cont;

  P4EST_ASSERT (p4est != NULL);
  P4EST_ASSERT (filename != NULL);

  /* Allocate, initialize the vtk context.  Important to zero all fields. */
  cont = P4EST_ALLOC_ZERO (charm_vtk_context_t, 1);

  cont->p4est = p4est;
  cont->filename = P4EST_STRDUP (filename);

  cont->scale = charm_vtk_scale;
  cont->continuous = charm_vtk_continuous;

  return cont;
}

void
charm_vtk_context_set_geom (charm_vtk_context_t * cont,
...
void
charm_vtk_context_set_continuous (charm_vtk_context_t * cont, int continuous)
{
  P4EST_ASSERT (cont != NULL);
  P4EST_ASSERT (!cont->writing);
...
void
charm_vtk_context_destroy (charm_vtk_context_t * context)
{
...
void
charm_vtk_write_file (p4est_t * p4est, p4est_geometry_t * geom,
                      const char *filename)
{
  int                 retval;
  charm_vtk_context_t *cont;
  ...
  cont = charm_vtk_context_new (p4est, filename);
  charm_vtk_context_set_geom (cont, geom);
...
charm_vtk_write_header (charm_vtk_context_t * cont)
{
...
#if defined CHARM_VTK_BINARY && defined CHARM_VTK_COMPRESSION
  ...
  float_data = P4EST_ALLOC (CHARM_VTK_FLOAT_TYPE, 3 * Npoints);

  /* write point position data */
  fprintf (cont->vtufile, "        <DataArray type=\"%s\" Name=\"Position\""
           " NumberOfComponents=\"3\" format=\"%s\">\n",
           CHARM_VTK_FLOAT_NAME, CHARM_VTK_FORMAT_STRING);
  ...
  if (nodes == NULL) {
    ...
    for (zz = 0; zz < indeps->elem_count; ++zz) {
      ...
      if (geom != NULL) {
        ...
        geom->X (geom, jt, xyz, XYZ);
        for (j = 0; j < 3; ++j) {
          float_data[3 * zz + j] = (CHARM_VTK_FLOAT_TYPE) XYZ[j];
        }
      }
      else {
        for (j = 0; j < 3; ++j) {
  ...
  fprintf (cont->vtufile,
           ...
           " format=\"%s\">\n", CHARM_VTK_LOCIDX, CHARM_VTK_FORMAT_STRING);
#ifdef CHARM_VTK_ASCII
  for (sk = 0, il = 0; il < Ncells; ++il) {
    fprintf (cont->vtufile, "         ");
    for (k = 0; k < P4EST_CHILDREN; ++sk, ++k) {
  ...
  if (nodes == NULL) {
    ...
    retval =
      charm_vtk_write_binary (cont->vtufile, (char *) locidx_data,
                              sizeof (p4est_locidx_t) * Ncorners);
    P4EST_FREE (locidx_data);
  }
  else {
    retval =
      charm_vtk_write_binary (cont->vtufile, (char *) nodes->local_nodes,
                              sizeof (p4est_locidx_t) * Ncorners);
  }
  fprintf (cont->vtufile, "\n");
  if (retval) {
  ...
  fprintf (cont->vtufile, "          ");
  retval = charm_vtk_write_binary (cont->vtufile, (char *) locidx_data,
  ...
  fprintf (cont->vtufile, "        <DataArray type=\"UInt8\" Name=\"types\""
           " format=\"%s\">\n", CHARM_VTK_FORMAT_STRING);
#ifdef CHARM_VTK_ASCII
  fprintf (cont->vtufile, "         ");
  for (il = 0, sk = 1; il < Ncells; ++il, ++sk) {
    fprintf (cont->vtufile, " %d", CHARM_VTK_CELL_TYPE);
  ...
  for (il = 0; il < Ncells; ++il)
    uint8_data[il] = CHARM_VTK_CELL_TYPE;

  fprintf (cont->vtufile, "          ");
  retval = charm_vtk_write_binary (cont->vtufile, (char *) uint8_data,
  ...
  if (mpirank == 0) {
     ...
     * only in charm_vtk_write_footer().
     */
 ...
 * the same as \b charm_vtk_write_point_dataf with the only difference being
 ...
 *
 * \note This function is actually called from \b charm_vtk_write_point_dataf
 ...
 *                  On failure, returns NULL and deallocates the context.
 */
static charm_vtk_context_t *
charm_vtk_write_point_datav (charm_vtk_context_t * cont,
                             ...
                             int num_point_vectors, va_list ap)
{
  ...
  list_end = va_arg (ap, charm_vtk_context_t *);
  ...
  all = 0;
  for (i = 0; i < num_point_scalars; ++all, ++i) {
    cont = charm_vtk_write_point_scalar (cont, names[all], values[all]);
    SC_CHECK_ABORT (cont != NULL,
                    P4EST_STRING "_vtk: Error writing point scalars");
  }

  for (i = 0; i < num_point_vectors; ++all, ++i) {
    cont = charm_vtk_write_point_vector (cont, names[all], values[all]);
    SC_CHECK_ABORT (cont != NULL,
...
charm_vtk_context_t *
charm_vtk_write_point_dataf (charm_vtk_context_t * cont,
                             ...
                             ...)
{
  ...
  cont = charm_vtk_write_point_datav (cont, num_point_scalars,
 ...
 * the same as \b charm_vtk_write_cell_dataf with the only difference being
 * that instead of a variable argument list, an initialized \a va_list is
 * passed as the last argument. The \a va_list is initialized from the variable
 * argument list of the calling function.
 *
 * \note This function is actually called from \b charm_vtk_write_cell_dataf
 * and does all of the work.
 *
 * \param [in,out] cont    A vtk context created by \ref charm_vtk_context_new.
 * \param [in] num_point_scalars Number of point scalar datasets to output.
 ...
 */
static charm_vtk_context_t *
charm_vtk_write_cell_datav (charm_vtk_context_t * cont,
                            ...
                            int num_cell_vectors, va_list ap)
{
  ...
  /* Check for pointer variable marking the end of variable data input. */
  charm_vtk_context_t *end = va_arg (ap, charm_vtk_context_t *);
  ...
  all = 0;
  for (i = 0; i < num_cell_scalars; ++all, ++i) {
    cont = charm_vtk_write_cell_scalar (cont, names[all], values[all]);
    SC_CHECK_ABORT (cont != NULL,
                    P4EST_STRING "_vtk: Error writing cell scalars");
  }

  for (i = 0; i < num_cell_vectors; ++all, ++i) {
    cont = charm_vtk_write_cell_vector (cont, names[all], values[all]);
    SC_CHECK_ABORT (cont != NULL,
...
charm_vtk_context_t *
charm_vtk_write_cell_dataf (charm_vtk_context_t * cont,
                            int write_tree, int write_level,
                            int write_rank, int wrap_rank,
                            int num_cell_scalars, int num_cell_vectors, ...)
{
  ...
  cont = charm_vtk_write_cell_datav (cont,
...
charm_vtk_context_t *
charm_vtk_write_point_scalar (charm_vtk_context_t * cont,
                              const char *scalar_name, sc_array_t * values)
{
  ...
  fprintf (cont->vtufile, "        <DataArray type=\"%s\" Name=\"%s\""
           " format=\"%s\">\n",
           CHARM_VTK_FLOAT_NAME, scalar_name, CHARM_VTK_FORMAT_STRING);
...
charm_vtk_context_t *
charm_vtk_write_point_vector (charm_vtk_context_t * cont,
...
charm_vtk_context_t *
charm_vtk_write_cell_scalar (charm_vtk_context_t * cont,
                             const char *scalar_name, sc_array_t * values)
{
  const p4est_locidx_t Ncells = cont->p4est->local_num_quadrants;
  p4est_locidx_t      il;
...
charm_vtk_context_t *
charm_vtk_write_cell_vector (charm_vtk_context_t * cont,
                             const char *vector_name, sc_array_t * values)
{
  P4EST_ASSERT (cont != NULL && cont->writing);
...
int
charm_vtk_write_footer (charm_vtk_context_t * cont)
{
  int                 p;
...
charm_vtk_context_t *charm_vtk_write_cell_data (charm_vtk_context_t * cont,
                                                int write_tree,
                                                int write_level,
                                                int write_rank,
                                                int wrap_rank,
...
//  sc_array_t        **values;
  ...
  //values = P4EST_ALLOC (sc_array_t *, num_cell_scalars + num_cell_vectors);
  names = filenames;//P4EST_ALLOC (const char *, num_cell_scalars + num_cell_vectors);
