...
static void charm_model_ns_conv_volume_int_iter_fn (p4est_iter_volume_info_t * info, void *user_data)
{
    ...
    for (ibf = 0; ibf < CHARM_BASE_FN_COUNT; ibf++) {
        for (igp = 0; igp < CHARM_QUAD_GP_COUNT; igp++) {
            ...
            for(cj = 0; cj < c_count; ++cj) {
                data->integrals.rc[cj][ibf] -= (fc[cj]*phi_x+gc[cj]*phi_y+hc[cj]*phi_z);
            }

            data->integrals.ru[ibf] -= ((fu*phi_x+gu*phi_y+hu*phi_z) + p.r*data->par.grav[0]*phi);
            data->integrals.rv[ibf] -= ((fv*phi_x+gv*phi_y+hv*phi_z) + p.r*data->par.grav[1]*phi);
            data->integrals.rw[ibf] -= ((fw*phi_x+gw*phi_y+hw*phi_z) + p.r*data->par.grav[2]*phi);
            data->integrals.re[ibf] -= ((fe*phi_x+ge*phi_y+he*phi_z));
        }
    }
    CHARM_FREE(fc);
    CHARM_FREE(gc);
