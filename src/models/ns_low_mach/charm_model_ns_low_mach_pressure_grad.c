...
static void _charm_model_ns_low_mach_pressure_grad_volume_int_iter_fn (p4est_iter_volume_info_t * info, void *user_data)
{
...
static void _charm_model_ns_low_mach_pressure_grad_surface_int_iter_bnd (p4est_iter_face_info_t * info, void *user_data) {
    int i, ibf, igp;
    p4est_t *p4est = info->p4est;
...
static void _charm_model_ns_low_mach_pressure_grad_surface_int_iter_inner (p4est_iter_face_info_t * info, void *user_data)
{
...
static void _charm_model_ns_low_mach_pressure_grad_surface_int_iter_fn (p4est_iter_face_info_t * info, void *user_data)
{
    sc_array_t         *sides = &(info->sides);

    if (sides->elem_count != 2) {
        _charm_model_ns_low_mach_pressure_grad_surface_int_iter_bnd(info, user_data);
    }
    else {
        _charm_model_ns_low_mach_pressure_grad_surface_int_iter_inner(info, user_data);
    }

}


static void _charm_model_ns_low_mach_pressure_grad_update_quad_iter_fn (p4est_iter_volume_info_t * info, void *user_data)
{
    charm_data_t       *data = charm_get_quad_data(info->quad);

    charm_matr_vect_mult(data->par.g.a_inv, data->int_grad_p_x, data->par.grad_p.x);
    charm_matr_vect_mult(data->par.g.a_inv, data->int_grad_p_y, data->par.grad_p.y);
    charm_matr_vect_mult(data->par.g.a_inv, data->int_grad_p_z, data->par.grad_p.z);
}


static void _charm_model_ns_low_mach_pressure_grad_zero_quad_iter_fn (p4est_iter_volume_info_t * info, void *user_data)
{
    charm_data_t       *data = charm_get_quad_data(info->quad);
    int                 i;

    for (i = 0; i < CHARM_BASE_FN_COUNT; i++) {
        data->int_grad_p_x[i] = 0.;
        data->int_grad_p_y[i] = 0.;
        data->int_grad_p_z[i] = 0.;
    }
}
...
void charm_model_ns_low_mach_pressure_grad(p4est_t * p4est, p4est_ghost_t * ghost, charm_data_t * ghost_data)
{
    p4est_iterate (p4est,
                   ghost,
                   (void *) ghost_data,
                   _charm_model_ns_low_mach_pressure_grad_zero_quad_iter_fn,
                   NULL, NULL, NULL);

    p4est_iterate (p4est,
                   ghost,
                   (void *) ghost_data,
                   _charm_model_ns_low_mach_pressure_grad_volume_int_iter_fn,
                   _charm_model_ns_low_mach_pressure_grad_surface_int_iter_fn,
                   NULL, NULL);

    p4est_iterate (p4est, NULL,
                   NULL,
                   _charm_model_ns_low_mach_pressure_grad_update_quad_iter_fn,
                   NULL, NULL, NULL);
