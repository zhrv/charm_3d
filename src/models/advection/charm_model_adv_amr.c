...
static int charm_coarsen_err_estimate (p4est_t * p4est, p4est_topidx_t which_tree, p4est_quadrant_t * children[])
{
    ...
    charm_cons_t        cons;
    charm_prim_t        prim;
    charm_real_t              vol, svol, err2, childerr2;
    ...
    for (i = 0; i < CHARM_CHILDREN; i++) {
        data = charm_get_quad_data(children[i]);
        vol  = charm_quad_get_volume(data);
        svol += vol;
        parentu += data->par.c.rc[0][0] * vol;
    }
    parentu /= svol;
    ...
    for (i = 0; i < CHARM_CHILDREN; i++) {
        ...
        err2 += childerr2;
        diff = (parentu - data->par.c.rc[0][0]) * (parentu - data->par.c.rc[0][0]);
        err2 += diff * vol;
    }
    if (err2 < global_err2 * (svol / CHARM_CHILDREN)) {
        return 1;
    }
    else {
        return 0;
                                 ...
                                 int num_incoming, p4est_quadrant_t * incoming[])
{
    ...
    if (num_outgoing > 1) { /* this is coarsening */
        ...
        parent_data->par.c.rc[0][0] = 0.;
        svol = 0.;
        for (i = 0; i < CHARM_CHILDREN; i++) {
            child_data = (charm_data_t *) outgoing[i]->p.user_data;
            vol = child_data->par.g.volume;
            svol += vol;
            parent_data->par.c.rc[0][0] += vol * child_data->par.c.rc[0][0];
            parent_data->par.c.rc[1][0] += vol * child_data->par.c.rc[1][0];
            for (j = 1; j < CHARM_BASE_FN_COUNT; j++) {
                parent_data->par.c.rc[0][j] = 0.;
                parent_data->par.c.rc[1][j] = 0.;
            }
        }
        parent_data->par.c.rc[0][0] /= svol;
    }
        ...
        for (i = 0; i < CHARM_CHILDREN; i++) {
            ...
            child_data->par.c.rc[0][0]  = parent_data->par.c.rc[0][0];
            child_data->par.c.rc[1][0]  = parent_data->par.c.rc[1][0];
            for (j = 1; j < CHARM_BASE_FN_COUNT; j++) {
                child_data->par.c.rc[0][j] = 0.;
                child_data->par.c.rc[1][j] = 0.;
            }
...
static void _charm_amr_par_calc_face_iter_bnd (p4est_iter_face_info_t * info, void *user_data)
{
    ...
    qr = udata->par.c.rc[0][0];

    for (ibf = 0; ibf < CHARM_DIM; ibf++) {
        udata->par.a.grad_u[ibf] += qr * s * n[ibf];
    }
    ...
    if (side[0]->is_hanging || side[1]->is_hanging) { // @todo
        for (j = 0; j < CHARM_HALF; j++) {
            ...
            for (ibf = 0; ibf < CHARM_DIM; ibf++) {
                for (i = 0; i < 2; i++) {
                    if (!side[i]->is.full.is_ghost) {
                        udata[i]->par.a.grad_u[ibf] += (i ? -1. : 1.) * s * 0.5*(udata[0]->par.c.rc[0][0]+udata[1]->par.c.rc[0][0]) * n[ibf];
                    }
                }
            }
        }
        ...
        qr = 0.5*(udata[0]->par.c.rc[0][0]+udata[1]->par.c.rc[0][0]);
        for (ibf = 0; ibf < CHARM_DIM; ibf++) {
            for (i = 0; i < 2; i++) {
                if (!side[i]->is.full.is_ghost) {
                    bfv = (i ? -1. : 1.) * s;
