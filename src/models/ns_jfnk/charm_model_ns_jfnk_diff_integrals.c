...
static void charm_model_ns_jfnk_diff_integrals_volume_int_iter_fn (p4est_iter_volume_info_t * info, void *user_data)
{
    ...
    for (ibf = 0; ibf < CHARM_BASE_FN_COUNT; ibf++) {
        for (igp = 0; igp < CHARM_QUAD_GP_COUNT; igp++) {
            ...
            data->int_r.ru[ibf] += (tau.xx*phi_x+tau.xy*phi_y+tau.xz*phi_z);
            data->int_r.rv[ibf] += (tau.xy*phi_x+tau.yy*phi_y+tau.yz*phi_z);
            data->int_r.rw[ibf] += (tau.xz*phi_x+tau.yz*phi_y+tau.zz*phi_z);

            data->int_r.re[ibf] += (tau.xx*p.u+tau.xy*p.v+tau.xz*p.w - qt[0])*phi_x;
            data->int_r.re[ibf] += (tau.xy*p.u+tau.yy*p.v+tau.yz*p.w - qt[1])*phi_y;
            data->int_r.re[ibf] += (tau.xz*p.u+tau.yz*p.v+tau.zz*p.w - qt[2])*phi_z;

            data->int_r.re[ibf] += data->par.model.ns_jfnk.chem_rhs*phi;
        }
    }
}
...
static void charm_model_ns_jfnk_diff_integrals_surface_int_iter_fn (p4est_iter_face_info_t * info, void *user_data)
{
    sc_array_t         *sides = &(info->sides);
...
void charm_model_ns_jfnk_dg_operator_diff_integrals(p4est_t * p4est, p4est_ghost_t * ghost, charm_data_t * ghost_data)
{
    p4est_iterate (p4est,
                   ghost,
                   (void *) ghost_data,
                   charm_model_ns_jfnk_diff_integrals_volume_int_iter_fn,
                   charm_model_ns_jfnk_diff_integrals_surface_int_iter_fn,
                   NULL, NULL);
