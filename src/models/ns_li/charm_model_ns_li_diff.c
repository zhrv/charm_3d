...
static void _charm_model_ns_li_timestep_diffusion_quad_iter_fn(p4est_iter_volume_info_t * info, void *user_data)
{
    ...
    memset(data->par.model.ns_li.d, 0, sizeof(double)*CHARM_MAX_COMPONETS_COUNT);

    if (!ctx->model.ns_li.use_diff) {
        return;
    }
    ...
    for (i = 0; i < c_count; i++) {
        if (fabs(prim.c[i]-1.) <= CHARM_EPS) {
            ...
            data->par.model.ns_li.d[i] = (1. - x[i]) / s_xd;
        }
    }
    CHARM_FREE(x);
}
...
void charm_model_ns_li_timestep_diffusion(p4est_t * p4est, p4est_ghost_t * ghost, charm_data_t * ghost_data)
{
    ...
    p4est_iterate (p4est, NULL, NULL,
                   _charm_model_ns_li_timestep_diffusion_quad_iter_fn,
                   NULL, NULL, NULL);
...
void charm_model_ns_li_timestep_diff(p4est_t * p4est, p4est_ghost_t * ghost, charm_data_t * ghost_data)
{
    ...
    if (!ctx->model.ns_li.use_visc) return;
    charm_model_ns_li_timestep_diffusion(p4est, ghost, ghost_data);
    charm_model_ns_li_timestep_diff_grad(p4est, ghost, ghost_data);
    charm_model_ns_li_timestep_diff_integrals(p4est, ghost, ghost_data);
}
