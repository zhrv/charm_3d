...
static void _charm_model_ns_li_diff_integrals_volume_int_iter_fn (p4est_iter_volume_info_t * info, void *user_data)
{
    ...
    for (ibf = 0; ibf < CHARM_BASE_FN_COUNT; ibf++) {
        for (igp = 0; igp < CHARM_QUAD_GP_COUNT; igp++) {
            ...
            data->int_re[ibf] += data->par.model.ns_li.chem_rhs*phi;
        }
    }
}
...
static void _charm_model_ns_li_diff_integrals_surface_int_iter_fn (p4est_iter_face_info_t * info, void *user_data)
{
    sc_array_t         *sides = &(info->sides);
...
void charm_model_ns_li_timestep_diff_integrals(p4est_t * p4est, p4est_ghost_t * ghost, charm_data_t * ghost_data)
{
    p4est_iterate (p4est,
                   ghost,
                   (void *) ghost_data,
                   _charm_model_ns_li_diff_integrals_volume_int_iter_fn,
                   _charm_model_ns_li_diff_integrals_surface_int_iter_fn,
                   NULL, NULL);
