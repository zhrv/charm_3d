...
static void _charm_model_ns_li_diff_grad_volume_int_iter_fn (p4est_iter_volume_info_t * info, void *user_data)
{
    ...
    for (ibf = 0; ibf < CHARM_BASE_FN_COUNT; ibf++) {
        for (igp = 0; igp < CHARM_QUAD_GP_COUNT; igp++) {
            ...
            for (i = 0; i < c_count; i++) {
                ...
                tmp_qx += h*p.r*data->par.model.ns_li.d[i]*p.c[i]*phi_x;
                tmp_qy += h*p.r*data->par.model.ns_li.d[i]*p.c[i]*phi_y;
                tmp_qz += h*p.r*data->par.model.ns_li.d[i]*p.c[i]*phi_z;
            }
...
static void _charm_model_ns_li_conv_surface_int_iter_bnd (p4est_iter_face_info_t * info, void *user_data) {
    int i, ibf, igp;
    ...
    for (igp = 0; igp < CHARM_FACE_GP_COUNT; igp++) {
        ...
        for (i = 0; i < c_count; i++) {
            ...
            qtx += h*fr*udata->par.model.ns_li.d[i]*fc*n[0];
            qty += h*fr*udata->par.model.ns_li.d[i]*fc*n[1];
            qtz += h*fr*udata->par.model.ns_li.d[i]*fc*n[2];
        }
        for (ibf = 0; ibf < CHARM_BASE_FN_COUNT; ibf++) {
            if (!side[0]->is.full.is_ghost) {
...
static void _charm_model_ns_li_conv_surface_int_iter_inner (p4est_iter_face_info_t * info, void *user_data)
{
        ...
        for (j = 0; j < CHARM_HALF; j++) {
            ...
            for (igp = 0; igp < CHARM_FACE_GP_COUNT; igp++) {
                ...
                for (i = 0; i < c_count; i++) {
                    ...
                    h = charm_comp_calc_enthalpy(comp, ft);
                    fc = (prim[0].c[i]+prim[1].c[i])*0.5;
                    dm = (udata[0]->par.model.ns_li.d[i]+udata[1]->par.model.ns_li.d[i])*0.5;
                    qtx += h*fr*dm*fc*n[0];
                    qty += h*fr*dm*fc*n[1];
                    qtz += h*fr*dm*fc*n[2];
                }
...
static void _charm_model_ns_li_diff_grad_surface_int_iter_fn (p4est_iter_face_info_t * info, void *user_data)
{
    sc_array_t         *sides = &(info->sides);

    if (sides->elem_count != 2) {
        _charm_model_ns_li_conv_surface_int_iter_bnd(info, user_data);
    }
    else {
        _charm_model_ns_li_conv_surface_int_iter_inner(info, user_data);
    }

}


static void _charm_model_ns_li_diff_grad_update_quad_iter_fn (p4est_iter_volume_info_t * info, void *user_data)
{
    charm_data_t       *data = charm_get_quad_data(info->quad);

    charm_matr_vect_mult(data->par.g.a_inv, data->int_tau_xx, data->par.model.ns_li.tau.xx);
    charm_matr_vect_mult(data->par.g.a_inv, data->int_tau_yy, data->par.model.ns_li.tau.yy);
    charm_matr_vect_mult(data->par.g.a_inv, data->int_tau_zz, data->par.model.ns_li.tau.zz);
    charm_matr_vect_mult(data->par.g.a_inv, data->int_tau_xy, data->par.model.ns_li.tau.xy);
    charm_matr_vect_mult(data->par.g.a_inv, data->int_tau_xz, data->par.model.ns_li.tau.xz);
    charm_matr_vect_mult(data->par.g.a_inv, data->int_tau_yz, data->par.model.ns_li.tau.yz);

    charm_matr_vect_mult(data->par.g.a_inv, data->int_q_x, data->par.model.ns_li.q.x);
    charm_matr_vect_mult(data->par.g.a_inv, data->int_q_y, data->par.model.ns_li.q.y);
    charm_matr_vect_mult(data->par.g.a_inv, data->int_q_z, data->par.model.ns_li.q.z);
}


static void _charm_model_ns_li_diff_grad_zero_quad_iter_fn (p4est_iter_volume_info_t * info, void *user_data)
{
    charm_data_t       *data = charm_get_quad_data(info->quad);
    int                 i;
...
void charm_model_ns_li_timestep_diff_grad(p4est_t * p4est, p4est_ghost_t * ghost, charm_data_t * ghost_data)
{
    p4est_iterate (p4est,
                   ghost,
                   (void *) ghost_data,
                   _charm_model_ns_li_diff_grad_zero_quad_iter_fn,
                   NULL, NULL, NULL);

    p4est_iterate (p4est,
                   ghost,
                   (void *) ghost_data,
                   _charm_model_ns_li_diff_grad_volume_int_iter_fn,
                   _charm_model_ns_li_diff_grad_surface_int_iter_fn,
                   NULL, NULL);

    p4est_iterate (p4est, NULL,
                   NULL,
                   _charm_model_ns_li_diff_grad_update_quad_iter_fn,
                   NULL, NULL, NULL);
