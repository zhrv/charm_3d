...
static void _charm_model_ns_li_chem_rhs(p4est_t * p4est, charm_data_t *data)
{
...
static void _charm_model_ns_li_chem_rhs_save(p4est_t * p4est, charm_data_t *data)
{
    double R = charm_eos_get_r();
    charm_cons_t cons;
    charm_prim_t prim;
    ...
    data->par.model.ns_li.chem_rhs = 0.;
    for (i = 0; i < c_count; i++) {
        w = 0.;
        for (n = 0; n < r_count; n++) {
            r = charm_get_reaction(p4est, n);
        ...
        data->par.model.ns_li.chem_rhs += w*charm_comp_calc_enthalpy(comp, prim.t);
    }

}


static void _charm_model_ns_li_chem_iter_fn(p4est_iter_volume_info_t * info, void *user_data)
{
    ...
    for (j = 0; j < 2; j++) {
        ...
        _charm_model_ns_li_chem_rhs(p4est, data);
    ...
    _charm_model_ns_li_chem_rhs_save(p4est, data);
...
static void _charm_model_ns_li_chem_zero_rhs_quad_iter_fn(p4est_iter_volume_info_t * info, void *user_data)
{
    charm_get_quad_data(info->quad)->par.model.ns_li.chem_rhs = 0.;
}
...
void charm_model_ns_li_timestep_chem(p4est_t * p4est, p4est_ghost_t * ghost, charm_data_t * ghost_data)
{
    ...
    if (!ctx->reactions) {
        p4est_iterate (p4est,
                       ghost,
                       (void *) ghost_data,
                       _charm_model_ns_li_chem_zero_rhs_quad_iter_fn,
                       NULL, NULL, NULL);
    ...
    p4est_iterate (p4est,
                   NULL, NULL,
                   _charm_model_ns_li_chem_iter_fn,
                   NULL, NULL, NULL);
