...
static void _charm_model_ns_li_chem_init_fetch_reaction(charm_ctx_t *ctx, YAML::Node node, charm_reaction_t *r)
{
    int i, comp;
...
static void _charm_model_ns_li_chem_init(charm_ctx_t *ctx, YAML::Node yaml)
{
    YAML::Node r_node;
    ...
    for (auto it : r_node) {
        ...
        _charm_model_ns_li_chem_init_fetch_reaction(ctx, it, r);
    }
}
...
void charm_model_ns_li_init(charm_ctx_t *ctx, YAML::Node model_node, YAML::Node yaml)
{
    ...
    YAML::Node sigmas;
    for (auto it: model_node["li_sigma"]) {
        auto *sig = (double *) sc_array_push(ctx->model.ns_li.li_sigma);
        *sig = it.as<double>();
    }

    _charm_model_ns_li_chem_init(ctx, yaml);
}
