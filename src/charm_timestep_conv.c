...
static void charm_convect_zero_quad_iter_fn (p4est_iter_volume_info_t * info, void *user_data)
{
...
static void charm_convect_update_quad_iter_fn (p4est_iter_volume_info_t * info, void *user_data)
{
    charm_data_t       *data = charm_get_quad_data(info->quad);
...
static void charm_convect_surface_int_iter_fn (p4est_iter_face_info_t * info, void *user_data)
{
    sc_array_t         *sides = &(info->sides);
...
void charm_timestep_conv(p4est_t *p4est, p4est_ghost_t *ghost, charm_data_t *ghost_data, double *dt)
{
    p4est_iterate (p4est,
                   ghost,
                   (void *) ghost_data,
                   charm_convect_zero_quad_iter_fn,
                   NULL, NULL, NULL);
...
//                                                                corners between quadrants */
    ...
    p4est_iterate (p4est, NULL,                              /* ghosts are not needed for this loop */
                   (void *) dt,                             /* pass in dt */
                   charm_convect_update_quad_iter_fn,       /* update each cell */
